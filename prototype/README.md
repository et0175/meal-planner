...
- A single-page app (all views in `frontend/app/page.tsx`) with in-memory seed data — no backend, no persistence, no user accounts
- Covers: meal planner, product/recipe browsing, product analyser, diets, shopping list, profile, search
...
Prototype only — dummy data, no persistence, no user management (no sign-in/register/accounts). Not connected to the backend microservices in `backend/`.
