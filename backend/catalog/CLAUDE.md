...
- **External import (FR-038, ADR-0013)**: global products are bulk-loaded from USDA FoodData Central (Foundation + SR Legacy) via `python -m importer --dir <fdc_csv_dir>`. `products.source`/`external_id` (migration 0003) with a partial `UNIQUE(source, external_id)` make re-import an idempotent upsert. Energy uses an Atwater fallback chain (nutrient 1008 → 2048 → 2047); missing macros default to 0.0; units capped at 10 (INV-004) with a canonical `100 g` base. Imported names are English-only; USDA has no diet labels so `diet_tags` are empty. User products keep `source=NULL` and are never touched by the importer.
