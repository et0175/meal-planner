...
**Test data:** User u-001 (`user@mealforge.com` / `password123`); user u-002 (`keto@mealforge.com` / `password123`); user u-003 (`empty@mealforge.com` / `password123`)  
**Test credentials hint:** The sign-in screen shows demo credentials for convenience.
...
**Status:** 🚫  
**Note (prototype):** "Forgot password?" form exists and shows an identical anti-enumeration message for any email. However no actual email is sent and there is no reset link flow — those parts remain 🚫.
...
**Status:** ✅  
**Note (prototype):** This is a single-page app with no URL-based routing. All app content is blocked at the app level when not signed in — the sign-in screen is the only thing visible. After sign-in the landing view is always Planner (no specific URL to redirect back to). The "redirect to originally requested URL" part is not applicable in the prototype.
...
**Status:** ✅  
**Note (prototype):** Session stored in `sessionStorage`; cleared on browser close or sign-out.
...
**Status:** ✅  
**Note (prototype):** All registered accounts are created with role `user`. No role distinction is enforced in the prototype UI — all users see identical navigation.
