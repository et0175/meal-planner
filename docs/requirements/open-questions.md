...
**Design consideration — smart default for the picker's day/slot:**
Whichever option is chosen, the picker should open pre-filled with a suggested day + meal slot rather than a blank/always-Lunch default, or it just reproduces the two-step friction it's meant to remove. The suggestion should be **frequency-weighted by recency**, not last-choice-wins: pure "most recent" lets a one-off exception (e.g. a birthday dinner) hijack the default for everything afterward, while pure "most frequent" can feel stale if a user's routine genuinely shifts. Weighting recent picks somewhat more heavily than old ones gives the stability of a habit-based default while still adapting when a pattern actually changes. Scope: per item, per user (a given product/recipe's own day+slot history for that user, not a global default). This needs to be part of whatever design is chosen, not an afterthought once the picker ships.
